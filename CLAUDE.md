...
**Branch Protection:** `main` requires the `CI Gate` status check to pass before merging. The `CI Gate` job in `.github/workflows/ci.yml` aggregates all conditional lint/test/type-check jobs — it fails if any upstream job fails or is cancelled, and passes if all succeed or are skipped. `enforce_admins` is off (admin can bypass in emergencies). No PR review required. CODEOWNERS file at `.github/CODEOWNERS` maps all paths to `@teatrie`.
